...
def plt_divergence(p_hist, J_hist, x_train, y_train):
    ...
    fig.suptitle("Cost escalation when learning rate is too large")
    ...
    fix_b = 100
    w_array = np.linspace(-70000, 70000, 1000)
    cost = np.zeros_like(w_array)

    for i, tmp_w in enumerate(w_array):
        cost[i] = compute_cost(x_train, y_train, tmp_w, fix_b)

    ax1.plot(w_array, cost)
    ax1.plot(x, v, c='magenta')
    ...
    tmp_b, tmp_w = np.meshgrid(np.linspace(-35000, 35000, 100), np.linspace(-70000, 70000, 100))
    z = np.zeros_like(tmp_b)
    for i in range(tmp_w.shape[0]):
        for j in range(tmp_w.shape[1]):
            z[i][j] = compute_cost(x_train, y_train, tmp_w[i][j], tmp_b[i][j])

    ax2.plot_surface(tmp_w, tmp_b, z, alpha=0.3, color='blue')
